...
public class StoreService {
    ...
    public StoreDTO storeUpdate(StoreDTO storeDTO) {
        //DTO를 이용해서 존재여부 확인->DTO를 Entity변환->저장->결과를 DTO변환후 반환
    ...
    public StoreDTO storeRead(Integer idx) {
        ...
        if (read.isPresent()) {//조회한 결과가 있으면
            StoreDTO storeDTO = modelMapper.map(read.get(), StoreDTO.class);
            return storeDTO;
        }
        //조회한 내용이 없으면
        return null;
    ...
    public Page<StoreDTO> storeList(Pageable pageable, String type, String keyword) {
        ...
        if (keyword != null) { //검색어가 존재하면
            if (type.equals('1')) {//type분류가 1이면, 총판명으로 검색할 때
                ...
                //              );
                storeEntities = storeRepository.searchStoreAll(keyword, storePage);
                ...
                //storeEntities = storeRepository.findByStoreContainingOrStoreIdContainingOrStoreChiefIdContainingOrStoreChiefContaining(
